...
class Balance extends BaseAdmin
{
    ...
    public function change($user_id, $type, $money, $source_id = "", $cate_money = 0)
    {
        ...
        if ($money != 0) {
            ...
            $balanceModel->save($data);  //为啥要now在save，是因为存在多个余额更新的时候，只会更新一条
            //上面保存好主体表，下面保存明细表
     ...
     * @return array
     */
    public function getBalanceList($user_id, string $order = '', int $page = 1, int $limit = 10, int $type = 0): array
    {
        $result = [
            'status' => true,
            'msg' => '获取成功',
     ...
     * @throws DbException
     */
    public function getExportList(array $post = []): array
    {
        $return_data = [
            'status' => false,
            'msg' => error_code(10025, true),

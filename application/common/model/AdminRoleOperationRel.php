 ...
 * 平台角色和权限关联关系表
 */
class AdminRoleOperationRel extends BaseAdmin
{
    public function savePerm($role_id, $operations)
    {
        $aoModel = new AdminOperation();
        $ararModel = new AdminRoleAddonsRel();
        $addonsOperations = $aoModel->addonsOperations();
        $addonsOperations = array_column($addonsOperations, 'name', 'id');

        //先删除此角色的所有权限
        $this->where(['admin_role_id' => $role_id])->delete();
        $ararModel->where(['admin_role_id' => $role_id])->delete();

        $data = [];     //admin_role_operation_rel表
        $data1 = [];    //admin_role_addons_rel表
        ...
        foreach ($operations as $k => $v) {
            ...
            //判断id是否在节点管理里，如果是，那么就加到admin_role_operation_rel表
            $info = $aoModel->where('id', $v['id'])->find();
            if ($info) {
    ...
    public function getTree($role_id = 0)
    {
        $aoModel = new AdminOperation();
        if ($role_id == 0) {
            $tree = $aoModel->menuTree($aoModel::MENU_SELLER);
        } else {
            $tree = $this->permTree($role_id, $aoModel::MENU_SELLER);
        }
        return $tree;
    }
     ...
     * @param $act_name
     */
    public function checkPerm($admin_id, $p_id, $cont_name, $act_name, $addon_name = "")
    {
        $result = [
            'status' => false,
            'data' => '',
        ...
        //如果是超级管理员，直接返回
        if ($admin_id == $adminModel::TYPE_SUPER_ID) {
            $result['status'] = true;
            return $result;
        }
        if ($addon_name == "") {
            return $this->checkOperationPerm($admin_id, $p_id, $cont_name, $act_name);
        } else {
            return $this->checkAddonsPerm($admin_id, $cont_name, $act_name, $addon_name);
        }
    }
     ...
     */
    private function checkOperationPerm($admin_id, $p_id, $cont_name, $act_name)
    {
        $result = [
            'status' => false,
            'data' => '',
        ...
        if ($aoModel->checkNeedPerm($p_id, $cont_name, $act_name)) {
            $result['status'] = true;
            return $result;
        }
        //到这里就说明是普通的管理员，那么就取所有的角色所对应的节点
        $list = $aoModel->adminOperation($admin_id);
        if (!$list) {
        ...
        $contOperation = $aoModel->where($contWhere)->find();
        if (!$contOperation) {
        ...
        $actOperation = $aoModel->where($actWhere)->find();
        if (!$actOperation) {
            return error_code(11089);
        }
        //查看是否是是关联权限，如果是关联权限去查找对应的关联操作的权限
        if ($actOperation['perm_type'] == $aoModel::PERM_TYPE_REL) {
            $actOperation = $aoModel->where(['id' => $actOperation['parent_menu_id']])->find();
            if (!$actOperation) {
     ...
     * @param $addon_name
     */
    private function checkAddonsPerm($admin_id, $cont_name, $act_name, $addon_name)
    {
        $result = [
            'status' => false,
            'data' => '',
        ...
        $aoModel = new AdminOperation();
        $admin_list = $aoModel->getAdminAddonsMenu($admin_id);        //当前用户插件节点
        $list = $aoModel->addonsOperations(); //所有插件节点
        //先查找控制器
        $cont_id = "";

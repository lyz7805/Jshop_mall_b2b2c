...
class Admin extends BaseAdmin
{
    ...
    public function tableData($post)
    {
        ...
        $list = $this
            ...
            ->alias('m')
            ->leftJoin('admin_role_rel mrr', 'mrr.admin_id = m.id')
            ->leftJoin('admin_role mr', 'mr.id = mrr.role_id')
    ...
    public function toAdd($data)
    {
        ...
        //判断是新增还是修改
        if (isset($data['id'])) {
            $adminInfo = $this->where(['id' => $data['id']])->find();
            if (!$adminInfo) {
            ...
            if (isset($data['password']) && !empty($data['password'])) {
                if (strlen($data['password']) <= 5 || strlen($data['password']) > 16) {
                    ...
                    $data['password'] = $this->enPassword($data['password'], $adminInfo['ctime']);
                }
            } else {
                unset($data['password']);
            }
            ...
            //判断用户名是否重复
            $adminInfo = $this->where(['username' => $data['username']])->find();
            if ($adminInfo) {
        ...
        //清空所有的旧角色
        $adminRoleRelModel->where(['admin_id' => $data['id']])->delete();
        ...
        if (isset($data['role_id'])) {
            ...
            $adminRoleRelModel->saveAll($data1);
        }
    ...
    public function toLogin($data)
    {
        ...
        // 校验验证码
        if (session('?admin_login_fail_num')) {
            if (session('admin_login_fail_num') >= config('b2b2c.admin_login_fail_num')) {
        ...
        if ($userInfo) {
            ...
            // 写失败次数到session
            if (session('?admin_login_fail_num')) {
                session('admin_login_fail_num', session('admin_login_fail_num') + 1);
            } else {
                session('admin_login_fail_num', 1);
            }
     ...
     * @return array|string
     */
    public function chengePwd($admin_id, $oldPassword, $newPassword)
    {
        $result = [
            'status' => false,
            'data'   => '',
        ...
        $info   = $this->where(['id' => $admin_id])->find();
        if (!$info) {
    ...
    private function setSession($userInfo)
    {
        ...
        session('admin', $userInfo->toArray());

        $userLogModel = new AdminUserLog(); // 添加登录日志

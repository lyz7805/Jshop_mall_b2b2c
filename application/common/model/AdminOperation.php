...
class AdminOperation extends BaseAdmin
{
    ...
    public function adminMenu($admin_id,$type = self::PERM_TYPE_SUB)
    {
        ...
        if(cache('?admin_operation_'.$admin_id) && $type == self::PERM_TYPE_SUB){         //如果有缓存，并且类型是1的话，就取缓存，因为类型是1主要是后台菜单上用，类型是2的话，主要是给别人分配权限的时候会用到
            ...
            $list = $this->adminOperation($admin_id,$type);

            $menuTree = $this->createTree($list,$parent_menu_id);        //构建菜单树
            //把插件的菜单也增加上去
            $this->addonsMenu($admin_id,$menuTree,$type);

            //存储
            if($type == self::PERM_TYPE_SUB){
                cache('admin_operation_'.$admin_id,$menuTree,3600);    //当为1时最常用
            }
        }
    ...
    //取管理员所对应的所有角色的所有节点信息
    public function adminOperation($admin_id,$type = self::PERM_TYPE_HALFSUB){
        ...
        if($admin_id == $adminModel::TYPE_SUPER_ID){
            ...
            $adminRoleRel = new AdminRoleRel();
            //取此管理员的所有角色
            $roles = $adminRoleRel->where('admin_id',$admin_id)->select();
            if(!$roles->isEmpty()){
                $roles = $roles->toArray();
            ...
            $list = $this
                ...
                ->field('o.*')
                ->alias('o')
                ->join(config('database.prefix').'admin_role_operation_rel mror', 'o.id = mror.operation_id')
                ->where('mror.admin_role_id','IN',$roles)
...
//    }
     ...
     * @param $adminMenu       当前登陆者的权限树，
     * @param $nodeList         当前角色的on的权限
     * @param int $level        层级
     * @param int $parentId     父id
     * @return array
     */
    public function setRoleAdminMenu($adminMenu,$nodeList,$level=1,$parentId=0){
        $dtreeData = [];
        foreach($adminMenu as $v){
            ...
            if(isset($v['children']) && $v['children']){
                $isLast = false;
                $children = $this->setRoleAdminMenu($v['children'],$nodeList,$level+1,$v['id']);
            }else{
                $isLast = true;
                $children = [];
            }
...
//            }
    ...
    private function addonsMenu($admin_id,&$tree,$type){
        $list = $this->getAdminAddonsMenu($admin_id);
        foreach($list as $v) {
            if ($v) {
                if (!is_array($v)) {
                    continue;
    ...
    //获得一个管理员的所有插件的菜单列表
    public function getAdminAddonsMenu($admin_id){
        $list = $this->addonsOperations();
        $adminModel = new Admin();
        ...
        if($admin_id != $adminModel::TYPE_SUPER_ID){
            ...
            $admin_list = $mrarModel
                ->distinct(true)
                ->where('admin_role_id','IN',$roles)
                ->select();
            if(!$admin_list->isEmpty()){
                $admin_list = $admin_list->toArray();
                $admin_list = array_column($admin_list,'menu_id');
            }else{
                $admin_list = [];
            }
            //把插件权限上，不在$list里的都删掉
